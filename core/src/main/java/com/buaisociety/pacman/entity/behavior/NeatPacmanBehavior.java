...
/***************************************************************FUNC START************************************************************************/
// Pellet density in four quadrants around Pacman
...
/**
 * Returns a one-hot encoded array indicating the direction of the nearest accessible pellet.
 * Uses BFS to find the closest pellet while accounting for walls and obstacles.
 * The array will be [1, 0, 0, 0] for forward, [0, 1, 0, 0] for left,
 * [0, 0, 1, 0] for right, and [0, 0, 0, 1] for behind.
 * If no pellet is found or the path is blocked, it returns [0, 0, 0, 0].
 */
    ...
    /**
     * Returns a one-hot encoded array indicating the direction to the nearest power pellet.
     * The array will be [1, 0, 0, 0] for forward, [0, 1, 0, 0] for left,
     * [0, 0, 1, 0] for right, and [0, 0, 0, 1] for behind.
     * If no pellet is found, it returns [0, 0, 0, 0].
     */
    // private float[] getDirectionToNearestPellet() {
    //     Vector2i pacmanTile = pacman.getTilePosition();
    //     Vector2i nearestPellet = findNearestPelletPosition();

    //     System.out.println("nearestPellet"+ nearestPellet);
        
    //     // Default encoding for no direction
    //     float[] directionEncoding = {0f, 0f, 0f, 0f}; 

    //     // Return if no pellet was found
    //     if (nearestPellet == null || pacmanTile == null) {
    //         return directionEncoding;
    //     }

    //     // Calculate the relative position of the pellet
    //     int dx = nearestPellet.x - pacmanTile.x;
    //     int dy = nearestPellet.y - pacmanTile.y;

    //     // Determine direction based on Pacman's current facing direction
    //     Direction pacmanDirection = pacman.getDirection();

    //     // Check for each primary direction (forward, left, right, behind)
    //     if (dx == pacmanDirection.getDx() && dy == pacmanDirection.getDy()) {
    //         directionEncoding[0] = 1f; // Forward
    //     } else if (dx == pacmanDirection.left().getDx() && dy == pacmanDirection.left().getDy()) {
    //         directionEncoding[1] = 1f; // Left
    //     } else if (dx == pacmanDirection.right().getDx() && dy == pacmanDirection.right().getDy()) {
    //         directionEncoding[2] = 1f; // Right
    //     } else if (dx == pacmanDirection.behind().getDx() && dy == pacmanDirection.behind().getDy()) {
    //         directionEncoding[3] = 1f; // Behind
    //     }
...
/***************************************************************FUNC END************************************************************************/
    ...
    public Direction getDirection(@NotNull Entity entity) {
        ...
        if (pacman.getMaze().getLevelManager().getLevel() == 9) {
        ...
        // float explorationRate = 0.1f; // Set a 5% chance for exploration

        // // Exploration: Randomly choose a valid direction with a certain probability
        ...
        //         // Return a random valid direction for exploration
        ...
        //     scoreModifier -=  visits * 100;  // 60 is a scaling factor; adjust as needed
        // }
